...
int EtoPPressure=0;
  ...
  #ifdef GUS
    float delta_radius = GUS_PRINTABLE_RADIUS;
  #else
  ...
  #ifdef GUS 
    float max_arm_length[3] = MAX_ARM_LENGTH;
    float shoulder_height[3] = SHOULDER_HEIGHT;
  #else // not GUS
...
////////////////////////////////////
// Missing NONLINEAR_BED_LEVELING //
////////////////////////////////////
...
#define HOMEAXIS_DO(LETTER) \
  ...
  if (axis==X_AXIS ? HOMEAXIS_DO(X) :
    ...
    plan_buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], feedrate/60, active_extruder);
    st_synchronize();
	enable_endstops(false); // Stop ignoring Z probe while moving up to the top microswitch again.
...
void process_commands()
{
  ...
  if(code_seen('G'))
  {
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      feedrate = 0.0;
          ...
          plan_set_position(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]);
		#ifdef GUS
          destination[X_AXIS] = GUS_ARM_LENGTH;
          destination[Y_AXIS] = GUS_ARM_LENGTH;
          destination[Z_AXIS] = GUS_ARM_LENGTH;
		#else
  ...
  else if(code_seen('M'))
  {
    ...
    #ifdef DELTA
	#ifdef GUS
	case 665: // M665 set GUS max_arm_length calibration values
      for(int8_t i=0; i < 3; i++)
      {
